...
contract PriceFeedManager is AccessControl {
    ...
    function getLatestPrice(CurrencyPair pair) public view returns (int256 price, uint8 decimals) {
        ...
        if (startedAt == 0) revert StalePriceData();              // Round not yet started
        if (answeredInRound < roundId) revert StalePriceData();   // Incomplete round
        if (block.timestamp - updatedAt > feed.heartbeat) revert StalePriceData(); // Stale
        ...
        returns (int256 price, uint8 decimals, uint256 updatedAt)
    {
        ...
        if (startedAt == 0) revert StalePriceData();
        if (answeredInRound < roundId) revert StalePriceData();
        if (block.timestamp - timestamp > feed.heartbeat) revert StalePriceData();
        if (answer <= 0) revert InvalidPrice();
