...
///      Bits 159-150: beforeInitialize, afterInitialize, beforeAddLiquidity,
...
contract HookDeployer {
    ...
    event HookDeployed(
        ...
        address indexed convexoPassport,
        bytes32 salt
    );

    /// @notice Deploy a CompliantLPHook with CREATE2
    /// @param poolManager The Uniswap V4 PoolManager address
    /// @param convexoPassport The Convexo Passport NFT contract address
    /// @param salt The salt for CREATE2 deployment
    /// @return hook The deployed hook address
    function deploy(IPoolManager poolManager, IConvexoPassport convexoPassport, bytes32 salt)
        external
        returns (CompliantLPHook hook)
    {
        hook = new CompliantLPHook{salt: salt}(poolManager, convexoPassport);
        emit HookDeployed(address(hook), address(poolManager), address(convexoPassport), salt);
    }
    ...
    function computeAddress(IPoolManager poolManager, IConvexoPassport convexoPassport, bytes32 salt)
        external
        view
        returns (address)
    {
        bytes32 bytecodeHash = keccak256(
            abi.encodePacked(type(CompliantLPHook).creationCode, abi.encode(poolManager, convexoPassport))
        );
    ...
    /// @dev Required permissions: beforeAddLiquidity (157), beforeRemoveLiquidity (155), beforeSwap (153)
    /// @param poolManager The Uniswap V4 PoolManager address
    /// @param convexoPassport The Convexo Passport NFT contract address
    ...
    /// @notice Validate that an address has the correct hook permission bits
    /// @dev Checks bits: beforeAddLiquidity (157), beforeRemoveLiquidity (155), beforeSwap (153)
